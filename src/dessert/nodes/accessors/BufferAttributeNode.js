...
/**
 * In earlier `dessert.js` versions it was only possible to define attribute data
 * on geometry level. With `BufferAttributeNode`, it is also possible to do this
 * on the node level.
 * ```js
 * const geometry = new DESSERT.PlaneGeometry();
 ...
 * }
 *
 * material.colorNode = bufferAttribute( new DESSERT.Float32BufferAttribute( colors, 3 ) );
 * ```
...
class BufferAttributeNode extends InputNode {
	...
	constructor( value, bufferType = null, bufferStride = 0, bufferOffset = 0 ) {
		...
		/**
		 * The usage property. Set this to `DESSERT.DynamicDrawUsage` via `.setUsage()`,
