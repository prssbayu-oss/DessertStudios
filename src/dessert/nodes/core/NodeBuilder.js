...
class NodeBuilder {
	...
	removeChain( node ) {
		...
		if ( lastChain !== node ) {
			...
			throw new Error( 'DESSERT.NodeBuilder: Invalid node chaining!' );
	...
	generateConst( type, value = null ) {
		...
		throw new Error( `DESSERT.NodeBuilder: Type '${type}' not found in generate constant attempt.` );
	...
	removeActiveStack( stack ) {
		...
		if ( this.activeStacks[ this.activeStacks.length - 1 ] === stack ) {
			...
			throw new Error( 'DESSERT.NodeBuilder: Invalid active stack removal.' );
	...
	getNodeUniform( uniformNode, type ) {
		...
		if ( node === undefined ) {
			...
			if ( type === 'float' || type === 'int' || type === 'uint' ) node = new NumberNodeUniform( uniformNode );
				...
				throw new Error( `DESSERT.NodeBuilder: Uniform "${ type }" not implemented.` );
	...
	getSignature() {
		...
		return `// Dessert.js r${ REVISION } - Node System\n`;
