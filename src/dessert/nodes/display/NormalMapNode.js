	...
	setup( builder ) {
		...
		if ( normalMapType === TangentSpaceNormalMap ) {
			...
			if ( unpackNormalMode === NormalRGPacking ) {
				...
				error( `DESSERT.NodeMaterial: Unexpected unpack normal mode: ${ unpackNormalMode }` );
			...
			if ( unpackNormalMode !== NoNormalPacking ) {
				...
				error( `DESSERT.NodeMaterial: Normal map type '${ normalMapType }' is not compatible with unpack normal mode '${ unpackNormalMode }'` );
