...
import { addMethodChaining, nodeArray, nodeObject, nodeObjects, float } from '../dsl/DSLCore.js';
import { error } from '../../utils.js';

/**
 * This module represents the call of a {@link FunctionNode}. Developers are usually not confronted
 * with this module since they use the predefined DSL syntax `wgslFn` and `glslFn` which encapsulate
 * this logic.
 *
 * @augments TempNode
 */
	...
	generate( builder ) {
		...
		if ( Array.isArray( parameters ) ) {
			...
			if ( parameters.length > inputs.length ) {
				...
				error( 'DSL: The number of provided parameters exceeds the expected number of inputs in \'Fn()\'.' );

				parameters.length = inputs.length;

			} else if ( parameters.length < inputs.length ) {

				error( 'DSL: The number of provided parameters is less than the expected number of inputs in \'Fn()\'.' );
			...
			for ( const inputNode of inputs ) {
				...
				if ( node !== undefined ) {
					...
					error( `DSL: Input '${ inputNode.name }' not found in \'Fn()\'.` );
