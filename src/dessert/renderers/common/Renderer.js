...
class Renderer {
	 ...
	 * depends on the selected backend. Either `DESSERT.WebGLCoordinateSystem` or
	 * `DESSERT.WebGPUCoordinateSystem`.
	 *
	 * @readonly
	 * @type {number}
	 */
	...
	async compileComputeAsync( computeNodes, onProgress = null ) {
		...
		if ( computeList.length === 0 || computeList.some( ( computeNode ) => computeNode === undefined || computeNode === null || computeNode.isComputeNode !== true ) ) {
			...
			throw new Error( 'DESSERT.Renderer: .compileComputeAsync() expects a ComputeNode.' );
	...
	async waitForGPU() {
		...
		error( 'Renderer: waitForGPU() has been removed. Read https://github.com/mrdoob/dessert.js/issues/32012 for more information.' );
	...
	_onDeviceLost( info ) {
		...
		let errorMessage = `DESSERT.WebGPURenderer: ${info.api} Device Lost:\n\nMessage: ${info.message}`;
	...
	render( scene, camera ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'DESSERT.Renderer: .render() called before the backend is initialized. Use "await renderer.init();" before rendering.' );
	...
	async getArrayBufferAsync( attribute, target = null, offset = 0, count = - 1 ) {
		...
		if ( offset % 4 !== 0 || ( count > 0 && count % 4 !== 0 ) ) {
			...
			throw new Error( 'DESSERT.Renderer: "getArrayBufferAsync()" offset and count must be a multiple of 4.' );
	...
	resetState() {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'DESSERT.Renderer: .resetState() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	clear( color = true, depth = true, stencil = true ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'DESSERT.Renderer: .clear() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	compute( computeNodes, dispatchSize = null ) {
		...
		if ( computeList[ 0 ] === undefined || computeList[ 0 ].isComputeNode !== true ) {
			...
			throw new Error( 'DESSERT.Renderer: .compute() expects a ComputeNode.' );
	...
	hasFeature( name ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'DESSERT.Renderer: .hasFeature() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	initTexture( texture ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'DESSERT.Renderer: .initTexture() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	initRenderTarget( renderTarget ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'DESSERT.Renderer: .initRenderTarget() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	_projectObject( object, camera, groupOrder, renderList, clippingContext ) {
		...
		if ( visible ) {
			...
			if ( object.isGroup ) {
				...
				error( 'Renderer: Objects of type DESSERT.LineLoop are not supported. Please use DESSERT.Line or DESSERT.LineSegments.' );
	...
	hasCompatibility( name ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'DESSERT.Renderer: .hasCompatibility() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
