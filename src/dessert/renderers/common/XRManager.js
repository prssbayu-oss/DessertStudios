...
class XRManager extends EventDispatcher {
	...
	/**
	 * Returns an instance of `DESSERT.Group` that represents the transformation
	...
	_validateWebGPUSession() {
		...
		if ( this._session.enabledFeatures.includes( 'webgpu' ) === false ) {
			...
			throw new Error( 'DESSERT.XRManager: WebGPU XR sessions require the "webgpu" session feature. Use VRButtonGPU/XRButton with "webgpu" enabled or use a WebGL backend.' );
	...
	async _initWebGPUSession( session ) {
		...
		if ( this._useMultiviewIfPossible === true ) {
			...
			warnOnce( 'DESSERT.XRManager: WebGPU XR does not support multiview yet. Disabling multiview for this XR session.' );
