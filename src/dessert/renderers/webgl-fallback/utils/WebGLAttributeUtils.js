...
class WebGLAttributeUtils {
	...
	createAttribute( attribute, bufferType ) {
		...
		if ( array instanceof Float32Array ) {
			...
			throw new Error( 'DESSERT.WebGLBackend: Unsupported buffer data format: ' + array );
	...
	async getArrayBufferAsync( attribute, target = null, offset = 0, count = - 1 ) {
		...
		if ( target === null ) {
			...
			if ( target._mapped === true ) {
				...
				throw new Error( 'DESSERT.WebGPURenderer: ReadbackBuffer must be released before being used again.' );
