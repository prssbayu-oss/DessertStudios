	...
	constructor( parameters = {} ) {
		...
		if ( context !== null ) {
			...
			if ( typeof WebGLRenderingContext !== 'undefined' && context instanceof WebGLRenderingContext ) {
				...
				throw new Error( 'DESSERT.WebGLRenderer: WebGL 1 is not supported since r163.' );
		...
		try {
			...
			if ( 'setAttribute' in canvas ) canvas.setAttribute( 'data-engine', `dessert.js r${REVISION}` );
			...
			if ( _gl === null ) {
				...
				if ( _gl === null ) {
					...
					if ( getContext( contextName ) ) {
						...
						throw new Error( 'DESSERT.WebGLRenderer: Error creating WebGL context with your selected attributes.' );

					} else {

						throw new Error( 'DESSERT.WebGLRenderer: Error creating WebGL context.' );
		...
		this.render = function ( scene, camera ) {
			...
			if ( camera !== undefined && camera.isCamera !== true ) {
				...
				error( 'WebGLRenderer.render: camera is not an instance of DESSERT.Camera.' );
				return;
		...
		function setProgram( camera, scene, geometry, material, object ) {
			...
			if ( refreshProgram || _currentCamera !== camera ) {
				...
				// consider moving isOrthographic to UniformLib and WebGLMaterials, see https://github.com/mrdoob/dessert.js/pull/26467#issuecomment-1645185067
				...
				if ( renderTargetProperties.__useDefaultFramebuffer !== undefined ) {
					...
					if ( renderTargetProperties.__boundDepthTexture !== depthTexture ) {
						...
						if (
							...
							throw new Error( 'DESSERT.WebGLRenderer: Attached DepthTexture is initialized to the incorrect size.' );
		...
		this.readRenderTargetPixels = function ( renderTarget, x, y, width, height, buffer, activeCubeFaceIndex, textureIndex = 0 ) {
			...
			if ( ! ( renderTarget && renderTarget.isWebGLRenderTarget ) ) {
				...
				error( 'WebGLRenderer.readRenderTargetPixels: renderTarget is not DESSERT.WebGLRenderTarget.' );
				return;
		...
		this.readRenderTargetPixelsAsync = async function ( renderTarget, x, y, width, height, buffer, activeCubeFaceIndex, textureIndex = 0 ) {
			...
			if ( ! ( renderTarget && renderTarget.isWebGLRenderTarget ) ) {
				...
				throw new Error( 'DESSERT.WebGLRenderer.readRenderTargetPixels: renderTarget is not DESSERT.WebGLRenderTarget.' );
			...
			if ( framebuffer ) {
				...
				if ( ( x >= 0 && x <= ( renderTarget.width - width ) ) && ( y >= 0 && y <= ( renderTarget.height - height ) ) ) {
					...
					if ( readableState.__formatReadable === false ) {
						...
						throw new Error( 'DESSERT.WebGLRenderer.readRenderTargetPixelsAsync: renderTarget is not in RGBA or implementation defined format.' );

					}

					if ( readableState.__typeReadable === false ) {

						throw new Error( 'DESSERT.WebGLRenderer.readRenderTargetPixelsAsync: renderTarget is not in UnsignedByteType or implementation defined type.' );
					...
					throw new Error( 'DESSERT.WebGLRenderer.readRenderTargetPixelsAsync: requested read bounds are out of range.' );
		...
		if ( typeof __DESSERT_DEVTOOLS__ !== 'undefined' ) {

			__DESSERT_DEVTOOLS__.dispatchEvent( new CustomEvent( 'observe', { detail: this } ) );
