...
class WebGPUTextureUtils {
	...
	createTexture( texture, options = {} ) {
		...
		if ( textureData.initialized ) {
			...
			throw new Error( 'DESSERT.WebGPUTextureUtils: Texture already initialized.' );
	...
	async copyTextureToBuffer( texture, x, y, width, height, faceIndex ) {
		...
		_bufferDescriptor.size = ( ( height - 1 ) * bytesPerRow ) + ( width * bytesPerTexel ); // see https://github.com/mrdoob/dessert.js/issues/31658#issuecomment-3229442010
	...
	/**
	 * Converts the dessert.js uv wrapping constants to GPU address mode constants.
	 *
	 * @private
	 * @param {number} value - The dessert.js constant defining a uv wrapping mode.
	...
	/**
	 * Converts the dessert.js filter constants to GPU filter constants.
	 *
	 * @private
	 * @param {number} value - The dessert.js constant defining a filter mode.
	...
	/**
	 * Converts the dessert.js filter constants to a GPU mipmap filter constant.
	 * Unlike `_convertFilterMode`, this extracts the between-mip-level filtering
	 * axis from the combined dessert.js constant rather than the within-level axis.
	 *
	 * @private
	 * @param {number} value - The dessert.js constant defining a filter mode.
