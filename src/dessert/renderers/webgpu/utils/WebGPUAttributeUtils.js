	...
	async getArrayBufferAsync( attribute, target = null, offset = 0, count = - 1 ) {
		...
		if ( target !== null && target.isReadbackBuffer ) {
			...
			if ( target._mapped === true ) {
				...
				throw new Error( 'DESSERT.WebGPUAttributeUtils: ReadbackBuffer must be released before being used again.' );
		...
		if ( itemSize === 1 ) {
			...
			if ( prefix ) {
				...
				if ( paddedItemSize % 1 ) {
					...
					throw new Error( 'DESSERT.WebGPUAttributeUtils: Bad vertex format item size.' );
