 ...
 * display style set to 'block', which is commonly used in dessert.js
 ...
 * and console.error calls made by dessert.js, which is useful for custom logging,
 ...
 * method name and is automatically prefixed with 'DESSERT.'.
 *
...
function enhanceLogMessage( params ) {
	...
	if ( typeof message === 'string' && message.startsWith( 'DSL:' ) ) {
		...
		if ( stackTrace && stackTrace.isStackTrace ) {
			...
			params[ 1 ] = 'Stack trace not available. Enable "DESSERT.Node.captureStackTrace" to capture stack traces.';
