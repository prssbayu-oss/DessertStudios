...
/**
 * A loader for loading a JSON resource in the [JSON Object/Scene format](https://github.com/mrdoob/dessert.js/wiki/JSON-Object-Scene-format-4).
 * The files are internally loaded via {@link FileLoader}.
 *
 * ```js
 * const loader = new DESSERT.ObjectLoader();
...
class ObjectLoader extends Loader {
	...
	load( url, onLoad, onProgress, onError ) {
		...
		loader.load( url, function ( text ) {
			...
			if ( metadata === undefined || metadata.type === undefined || metadata.type.toLowerCase() === 'geometry' ) {
				...
				if ( onError !== undefined ) onError( new Error( 'DESSERT.ObjectLoader: Can\'t load ' + url ) );
	...
	async loadAsync( url, onProgress ) {
		...
		try {
			...
			throw new Error( 'DESSERT.ObjectLoader: Can\'t parse ' + url + '. ' + e.message );
		...
		if ( metadata === undefined || metadata.type === undefined || metadata.type.toLowerCase() === 'geometry' ) {
			...
			throw new Error( 'DESSERT.ObjectLoader: Can\'t load ' + url );
