 ...
 *
 * ```js
 * const box = new DESSERT.BoxGeometry( 1, 1, 1 );
 * const sphere = new DESSERT.SphereGeometry( 1, 12, 12 );
 * const material = new DESSERT.MeshBasicMaterial( { color: 0x00ff00 } );
 *
...
class BatchedMesh extends Mesh {
	...
	_validateGeometry( geometry ) {
		...
		if ( Boolean( geometry.getIndex() ) !== Boolean( batchGeometry.getIndex() ) ) {
			...
			throw new Error( 'DESSERT.BatchedMesh: All geometries must consistently have "index".' );
		...
		for ( const attributeName in batchGeometry.attributes ) {
			...
			if ( ! geometry.hasAttribute( attributeName ) ) {
				...
				throw new Error( `DESSERT.BatchedMesh: Added geometry missing "${ attributeName }". All geometries must have consistent attributes.` );
			...
			if ( srcAttribute.itemSize !== dstAttribute.itemSize || srcAttribute.normalized !== dstAttribute.normalized ) {
				...
				throw new Error( 'DESSERT.BatchedMesh: All attributes must have a consistent itemSize and normalized value.' );
	...
	validateInstanceId( instanceId ) {
		...
		if ( instanceId < 0 || instanceId >= instanceInfo.length || instanceInfo[ instanceId ].active === false ) {
			...
			throw new Error( `DESSERT.BatchedMesh: Invalid instanceId ${instanceId}. Instance is either out of range or has been deleted.` );
	...
	validateGeometryId( geometryId ) {
		...
		if ( geometryId < 0 || geometryId >= geometryInfoList.length || geometryInfoList[ geometryId ].active === false ) {
			...
			throw new Error( `DESSERT.BatchedMesh: Invalid geometryId ${geometryId}. Geometry is either out of range or has been deleted.` );
	...
	addInstance( geometryId ) {
		...
		if ( atCapacity && this._availableInstanceIds.length === 0 ) {
			...
			throw new Error( 'DESSERT.BatchedMesh: Maximum item count reached.' );
	...
	addGeometry( geometry, reservedVertexCount = - 1, reservedIndexCount = - 1 ) {
		...
		if (
			...
			throw new Error( 'DESSERT.BatchedMesh: Reserved space request exceeds the maximum buffer size.' );
	...
	setGeometryAt( geometryId, geometry ) {
		...
		if ( geometryId >= this._geometryCount ) {
			...
			throw new Error( 'DESSERT.BatchedMesh: Maximum geometry count reached.' );
		...
		if (
			...
			throw new Error( 'DESSERT.BatchedMesh: Reserved space not large enough for provided geometry.' );
	...
	*/
	setInstanceCount( maxInstanceCount ) {
		...
		if ( maxInstanceCount < instanceInfo.length ) {
			...
			throw new Error( `DESSERT.BatchedMesh: Instance ids outside the range ${ maxInstanceCount } are being used. Cannot shrink instance count.` );
	...
	*/
	setGeometrySize( maxVertexCount, maxIndexCount ) {
		...
		if ( requiredVertexLength > maxVertexCount ) {
			...
			throw new Error( `DESSERT.BatchedMesh: Geometry vertex values are being used outside the range ${ maxIndexCount }. Cannot shrink further.` );
		...
		if ( this.geometry.index ) {
			...
			if ( requiredIndexLength > maxIndexCount ) {
				...
				throw new Error( `DESSERT.BatchedMesh: Geometry index values are being used outside the range ${ maxIndexCount }. Cannot shrink further.` );
